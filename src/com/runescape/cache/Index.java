...
public class Index {
	...
	public synchronized byte[] get(int index) {
		try {
			...
			while (read < fileSize) {
				if (fileBlock == 0) {
					System.out.println("ZEROFILEBLOCK");
					return null;
				}
				...
				if (nextFileId != index || currentPartId != cycles || nextStoreId != storeId) {
					System.out.println("SOMESHITWRONGWITHNEXTFILE"+(nextFileId != index)+" "+(currentPartId != cycles)+" "+(nextStoreId != storeId));
					return null;
				}
				if (nextBlockId < 0 || nextBlockId > dataFile.length() / 520L) {
					System.out.println("NEXTBLOCKOUTOFRANGE");
					return null;
				}
